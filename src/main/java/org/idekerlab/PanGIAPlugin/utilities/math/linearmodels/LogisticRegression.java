...
public class LogisticRegression
{
	...
	{
		if (x.length != y.length) throw new IllegalArgumentException("Dimension mismatch: Xrows!=ylength, "+x.length+"!="+y.length);
        ...
        if (DoubleVector.anyGreaterThan(y,1) || DoubleVector.anyLessThan(y,0)) throw new IllegalArgumentException("y values must be 0 <= y <= 1");
