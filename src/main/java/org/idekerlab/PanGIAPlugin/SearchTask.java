...
public class SearchTask extends AbstractTask
{
			...
			throws IllegalArgumentException, ClassCastException
	{
		...
		else
		{
			...
			for (final CyEdge e : edges)
			{
				...
				{ // Assume we have an integer attribute.
	...
	private float[] scaleEdgeAttribValues(final float[] edgeAttribValues, final ScalingMethod scalingMethod, final StringBuilder errorMessage)
	{
		...
		try
		{
			scaledEdgeAttribValues = ScalarFactory.getScaler(scalingType).scale(edgeAttribValues, from, to);
		}
	...
	public void run(TaskMonitor taskMonitor) throws Exception
	{
		...
		if (parameters.getComplexAnnotation())
		{
			...
			for (SNodeModule complex : trainingComplexes)
			{
				...
				if (bestNode != null && bestScore >= parameters.getAnnotationThreshold())
					module_name.put(bestNode, complex.getID());
			}
